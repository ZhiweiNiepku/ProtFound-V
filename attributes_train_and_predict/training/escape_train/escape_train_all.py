...
if __name__ == '__main__':
    ...
    val_rbd_feats=np.load('data/RBD_embedding_val.npy')
    val_hseq_feats=np.load('data/HSEQ_embedding_val.npy')
    val_lseq_feats=np.load('data/LSEQ_embedding_val.npy')
    val_reg_label=np.load('data/label_val.npy').reshape(-1,1)
    val_cls_label=(val_reg_label>thres)
    
    if SCALE:
    
        mean_rbd=np.load('model/RBD_embedding_flatten_mean.npy')
        std_rbd=np.load('model/RBD_embedding_flatten_std.npy')
        mean_hseq=np.load('model/HSEQ_embedding_flatten_mean.npy')
        std_hseq=np.load('model/HSEQ_embedding_flatten_std.npy')
        mean_lseq=np.load('model/LSEQ_embedding_flatten_mean.npy')
        std_lseq=np.load('model/LSEQ_embedding_flatten_std.npy')
    
    
        shape_=val_rbd_feats.shape
        val_rbd_feats=val_rbd_feats.reshape(shape_[0],-1)
        val_rbd_feats=((val_rbd_feats-mean_rbd)/std_rbd).reshape(shape_)

        shape_=val_hseq_feats.shape
        val_hseq_feats=val_hseq_feats.reshape(shape_[0],-1)
        val_hseq_feats=((val_hseq_feats-mean_hseq)/std_hseq).reshape(shape_)

        shape_=val_lseq_feats.shape
        val_lseq_feats=val_lseq_feats.reshape(shape_[0],-1)
        val_lseq_feats=((val_lseq_feats-mean_lseq)/std_lseq).reshape(shape_)
    ...
    for i_ in range(run_round):
        ...
        train_rbd_feats=np.load('data/RBD_embedding_train-{}.npy'.format(i_))
        train_hseq_feats=np.load('data/HSEQ_embedding_train-{}.npy'.format(i_))
        train_lseq_feats=np.load('data/LSEQ_embedding_train-{}.npy'.format(i_))
        train_reg_label=np.load('data/label_train-{}.npy'.format(i_)).reshape(-1,1)
        ...
        Val_dataset = ScaleDataset(val_rbd_feats,val_hseq_feats,val_lseq_feats,val_cls_label,val_reg_label)
